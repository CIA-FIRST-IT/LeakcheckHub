...
def schedules_page(user: User, schedules: tuple[Schedule, ...]) -> str:
    ...
    content = "".join(
        (
            ...
            "<th>Next run</th><th>Latest results</th><th>Status</th><th>Actions</th></tr></thead>",
...
def _row(schedule: Schedule) -> str:
    ...
    latest = (
        f'<a href="/analyst/batches/{schedule.last_batch_id}">View latest results</a>'
        if schedule.last_batch_id is not None
        else "Not run yet"
    )
    return "".join(
        (
            ...
            f"<td>{latest}</td><td>{'enabled' if schedule.enabled else 'disabled'}</td><td>",
